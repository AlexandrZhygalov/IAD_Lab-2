...
print("\nАпостерні ймовірності:")
def calculate_posterior_probabilities(y_true, y_pred):
    residuals = y_true - y_pred
    residual_std = np.std(residuals)

    # Використання нормального розподілу для оцінки ймовірності
    posterior_probabilities = 1 - np.exp(-((y_true - y_pred) ** 2) / (2 * residual_std ** 2))

    return posterior_probabilities

# Розрахунок для набору даних "Кільця"
posterior_probabilities_circles = calculate_posterior_probabilities(y_val_circles, y_pred_circles)
mean_posterior_circles = np.mean(posterior_probabilities_circles)
print(f"Середня апостеріорна ймовірність для Circles: {mean_posterior_circles:.4f}")

# Розрахунок для набору даних "Ірис"
posterior_probabilities_iris = calculate_posterior_probabilities(y_val_iris, y_pred_iris)
mean_posterior_iris = np.mean(posterior_probabilities_iris)
print(f"Середня апостеріорна ймовірність для Iris: {mean_posterior_iris:.4f}")
